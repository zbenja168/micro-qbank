...
function AppShell() {
  ...
  const [tier, setTierState] = useState<Tier>(
    () => (localStorage.getItem('micro_qbank_tier') === 'advanced' ? 'advanced' : 'standard'),
  );
  const setTier = useCallback((t: Tier) => {
    try { localStorage.setItem('micro_qbank_tier', t); } catch { /* ignore */ }
    setTierState(t);
    setPage('home');
  }, []);
  ...
  const handleStartQuiz = useCallback(async () => {
    // extrasTopicIds is the trailing argument here: this repo's loadQuestions
    // takes the tier third, so the split rides after excludeIds.
